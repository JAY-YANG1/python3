...
tot, mean, grd = computSungJuk(kor, eng, mat)
...
print(redCar.size, redCar.color, redCar.wheels,
      redCarDoors, redCar.isLoad)

# 객체명만 호출하면 클래스에 정의해둔
# __str__ 가 호출되어 결과 출력
print(redCar)
...
class Employee:
    ...
    def __str__(self):
        result = f'{self.empno} {self.lname} {self.fname} {self.email} {self.phone}' \
                 f'{self.hdate} {self.jobid} {self.sal} {self.comm} {self.mgr}' \
                 f'{self.deptid}'
        return result
